...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_MAPPOOL_CREATURE_TEMPLATE, "SELECT poolId, phaseMask, spawnMask, minLimit, maxLimit, MovementType, spawnDist, spawntimeSecsMin, spawntimeSecsMax, corpsetimeSecsLoot, corpsetimeSecsNoLoot, description FROM mappool_creature_template WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_GAMEOBJECT_TEMPLATE, "SELECT poolId, phaseMask, spawnMask, minLimit, maxLimit, spawntimeSecsMin, spawntimeSecsMax, description FROM mappool_gameobject_template WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_CREATURE_HIERARCHY, "SELECT poolId, childPoolId FROM mappool_creature_hierarchy WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_GAMEOBJECT_HIERARCHY, "SELECT poolId, childPoolId FROM mappool_gameobject_hierarchy WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_CREATURE_SPAWNS, "SELECT poolId, pointId, zoneId, areaId, positionX, positionY, positionZ, orientation, gridId, MovementTypeOverride, spawnDistOverride, spawntimeSecsMinOverride, spawntimeSecsMaxOverride, corpsetimeSecsLootOverride, corpsetimeSecsNoLootOverride, AINameOverrideEntry, AINameOverride, ScriptNameOverrideEntry, ScriptNameOverride FROM mappool_creature_spawns WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_GAMEOBJECT_SPAWNS, "SELECT poolId, pointId, zoneId, areaId, positionX, positionY, positionZ, orientation, rotation0, rotation1, rotation2, rotation3, gridId, spawntimeSecsMinOverride, spawntimeSecsMaxOverride, AINameOverrideEntry, AINameOverride, ScriptNameOverrideEntry, ScriptNameOverride FROM mappool_gameobject_spawns WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_CREATURE_INFO, "SELECT poolId, creatureId, creatureQualifier, chance, modelId, equipmentId, currentWaypoint, curHealth, curMana, npcFlag, unitFlags, dynamicFlags, MovementTypeOverride, spawnDistOverride, spawntimeSecsMinOverride, spawntimeSecsMaxOverride, corpsetimeSecsLootOverride, corpsetimeSecsNoLootOverride, AINameOverride, ScriptNameOverride FROM mappool_creature_info WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_MAPPOOL_GAMEOBJECT_INFO, "SELECT poolId, gameobjectId, gameobjectQualifier, chance, spawntimeSecsMinOverride, spawntimeSecsMaxOverride, animProgress, state, AINameOverride, ScriptNameOverride FROM mappool_gameobject_info WHERE map = ?", CONNECTION_SYNCH);
    PrepareStatement(WORLD_UPD_MAPPOOL_CREATURE_ZONE_GRID_DATA, "UPDATE mappool_creature_spawns SET gridId = ? WHERE map = ? AND poolId = ? AND pointId = ?", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_MAPPOOL_GAMEOBJECT_ZONE_GRID_DATA, "UPDATE mappool_gameobject_spawns SET gridId = ? WHERE map = ? AND poolId = ? AND pointId = ?", CONNECTION_ASYNC);
}
